...
def get_credentials():
	...
	flow = InstalledAppFlow.from_client_secrets_file('esther_local_oath.json', SCOPES)
...
def get_subject_sender_receiver_date(headers, target_text):
	...
	sent_time = datetime.fromtimestamp(int(internal_date) / 1000).strftime('%Y-%m-%d %H:%M:%S')			
...
def not_replied_emails(creds):
	...
	# output data storage
	df = pd.DataFrame(columns=['subject', 'thread_id', 'sender', 'receiver','sent_time', 'body'])
		...
		if follow_up_label_id in txt['labelIds']:
			...
			try:
				...
				if (last_sender == sender) and (thread_id not in thread_ids):
					...
					new_row = {'subject': subject, 'thread_id': thread_id, 'sender': sender, 'receiver': receiver, 'sent_time': sent_time, 'body': body}
...
def send_email_to_all(creds, openai_json):
    ...
    for i in range(len(df)):
        ...
        subject = '-- Follow up reminder -- '+ df['subject'][i] + ' -- For ' + real_receiver + ' ---'
        print(subject)
        message = df['reply'][i]
        send_one_email(creds, sender, receiver, subject, message)
...
if __name__ == '__main__':
    ...
    send_email_to_all(creds, openai_json)
